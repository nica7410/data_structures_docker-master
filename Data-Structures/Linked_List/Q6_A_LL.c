...
int moveMaxToFront(ListNode **ptrHead)
{
    ...
    ListNode *maxCurNode = *ptrHead;
    ListNode *maxPrvNode = NULL;
	
	while (cur != NULL)
	{
		if (maxCurNode->item < cur->item)
		{
			maxPrvNode = prv;
			maxCurNode = cur;
		}
	...
	if (maxPrvNode == NULL) return 0;
	
	maxPrvNode->next = maxCurNode->next;
 	maxCurNode->next = *ptrHead;
    *ptrHead = maxCurNode;
